...
int testTrace(const char *trace) {
    ...
    if (system((string() + "cat " + trace + " | timeout 1 " + standerBasic + " > test_ans 2> /dev/null").c_str()) != 0)
        return 1;
    if (system((string() + "cat " + trace + " | timeout 1 " + studentBasic + " > test_out 2> /dev/null").c_str()) != 0)
        return 2;
    if (system("diff test_ans test_out > /dev/null 2> /dev/null"))
        return 4;
    if (system((string() + "cat " + trace + " | timeout 5 valgrind --error-exitcode=2 --leak-check=full " +
